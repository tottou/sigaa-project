...
public class ActionAgenda {
			...
			if (listaAgenda.get(i).getInicio()
				...
				while (bolz == true) {
					if (c.getTime().before(cc.getTime())) {
						...
						eventModel.addEvent(new EventoAgenda(
								listaAgenda.get(i), c.getTime(), c.getTime()));
					} else {
						bolz = false;
					}
				}
	...
	public void salvar() {
		...
		// if(event.getId() == null) {
		// eventModel.addEvent(new EventoAgenda(getAgenda(),
		// event.getStartDate(), event.getEndDate()));
		// }
		//
		//
		// else {
		// eventModel.updateEvent(new EventoAgenda(getAgenda(),
		// event.getStartDate(), event.getEndDate()));
		// }
		//
	...
	public void onDateSelect(DateSelectEvent selectEvent) {
		event = new EventoAgenda(new Agenda(), selectEvent.getDate(),
				selectEvent.getDate());
		setDiaSemana(setarDiaSemana(event.getStartDate()));
	}

	public void onEventMove(ScheduleEntryMoveEvent event) {
		EventoAgenda ea = (EventoAgenda) eventModel.getEvent(event
				.getScheduleEvent().getId());
		...
		FacesMessage message = new FacesMessage(FacesMessage.SEVERITY_INFO,
				"Agenda", " Data da sessão modificada");
		addMessage(message);
	}
	...
	public void onEventResize(ScheduleEntryResizeEvent event) {
		...
		AgendaDao.atualizarAgenda(ag);
		FacesMessage message = new FacesMessage(FacesMessage.SEVERITY_INFO,
				"Agenda", " Horário da sessão modificado");
	...
	public String getAjustaHora() {
		if (event.getStartDate().after(event.getEndDate())) {
			event.getEndDate().setTime(
					event.getStartDate().getTime() + (10 * 3600000));
			FacesContext context = FacesContext.getCurrentInstance();
			context.addMessage(
					null,
					new FacesMessage(
							"ERRO: Período inicial posterior ao final",
							"Correção realizada (a data pode ser modificada ao final do processo)"));
		}
		event.getStartDate().setTime(
				event.getStartDate().getTime() + (8 * 3600000));
		event.getEndDate()
				.setTime(event.getEndDate().getTime() + (9 * 3600000));
		return ajustaHora;
	}
